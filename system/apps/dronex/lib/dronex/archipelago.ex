defmodule Dronex.Archipelago do
  @moduledoc """
  ⚠ **COPIED FROM `Biotope.Archipelago`, DELIBERATELY, AND NOT EXTRACTED.**

  Two consumers is a copy. `DESIGN_THE_MAP.md` in `hecate-dronex` says so
  outright, and the house rule it is quoting is the same one that keeps the two
  islands' physics in their own images: a shared library here would make one
  track's map a reason not to change the other's, and these two worlds are going
  to diverge — the biotope draws a hex disc because its world is a hex disc, and
  this one draws airspace.

  The module is 165 lines, pure and tested, and contains nothing about biotope.
  What follows is its own documentation, unchanged.
