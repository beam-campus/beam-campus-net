defmodule Dronex.WatchBouts do
  @moduledoc """
  ...
  def handle_info(:subscribe, s) do
    {:noreply, attempt(Dronex.Mesh.configured?(), %{s | timer: nil})}
  end
  ...
  def handle_info({:macula_event_gone, ref, reason}, s) do
    Logger.warning("[Dronex] subscription gone (#{inspect(reason)}), re-arming")
  ...
  defp one(kind, pool, realm, s) do
    ...
    case :macula.subscribe(pool, realm, topic, self()) do
      {:ok, ref} ->
        Logger.info("[Dronex] subscribed #{topic}")
        %{s | subscribed: [kind | s.subscribed], refs: Map.put(s.refs, ref, kind)}

      other ->
        Logger.warning("[Dronex] subscribe #{topic} failed: #{inspect(other)}")
        s
    end
  end
  ...
  defp file(kind, payload, s) do
    ...
    Phoenix.PubSub.broadcast(@pubsub, @channel, {:dronex_changed, kind})
    s
  end
