defmodule Biotope.Mesh do
  ...
  ## Why this is not shared with `RoboRumbler.Mesh`

  It is nearly the same code, and the duplication is deliberate rather than
  overlooked. The two read different realms, and an independent pool each means a
  ...
  connection is cheaper than the coupling.

  If a third reader appears, consolidate then. Two is not a pattern.
