defmodule BeamCampusWeb.RedQueenLive do
  ...
  def mount(_params, _session, socket) do
    {:ok,
     assign(socket, page_title: "The Red Queen (numerical)", traj: nil, idx: 0, playing: false)}
  end
  ...
  def handle_event("play", _p, socket) do
    cond do
      ...
      socket.assigns.traj && socket.assigns.idx < length(socket.assigns.traj) ->
        {:noreply, start(socket)}

      true ->
        {:noreply, socket |> assign(traj: RedQueen.run(@gens), idx: 0) |> start()}
    end
  end
    ...
    <Layouts.app flash={@flash}>
      <div class="mx-auto max-w-4xl px-6 py-16">
        ...
        <div class="alert bg-base-200 border border-base-300 text-sm mb-6">
          <span>
            <b>This is not neuroevolution.</b>
            The players here are plain numbers, not networks,
            on purpose: it isolates the <b>coevolution measurement trap</b>
            where the true progress
            is a value you can read directly. It is the calibration of the ruler we use on the real
            engine next door. For coevolving networks, see the <.link
              navigate={~p"/research/workbench/neural-coevolution"}
              class="link"
            >neural coevolution demo</.link>.
          </span>
        </div>
        ...
        <p class="text-base-content/70 max-w-2xl mb-6">
          ...
          its <em>current</em>
          rivals never improves: it is always about as good relative to its peers,
        ...
        <div class="card bg-base-100 border border-base-300 mt-6">
          <div class="card-body gap-3">
            <p class="text-sm text-base-content/70">
              ...
              That is the Red Queen, and it is why coevolution must be measured against a <b>fixed benchmark</b>, not the moving opponent.
            </p>
            <div class="flex flex-wrap gap-x-6 gap-y-2 font-mono text-xs text-base-content/50 pt-1">
              ...
              <.link navigate={~p"/research/notes/running-to-stay-in-place"} class="link link-hover">
  ...
  defp chart(%{traj: nil} = assigns) do
    ~H|<div class="h-[300px] grid place-items-center text-base-content/40 font-mono text-xs">
  press Play to run the coevolution
</div>|
  end
  ...
  defp chart(assigns) do
    ...
    trait_pts =
      Enum.map_join(shown, " ", fn f -> "#{gx(f.gen, n)},#{ty(f.trait, tmin, tmax)}" end)
    ...
    <svg
      ...
      class="w-full h-auto"
      role="img"
      aria-label="The champion's trait rising while its score against current rivals stays flat."
    >
      <text
        x="86"
        y="34"
